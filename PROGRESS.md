...
- [ ] **M4 — Flagship 2: codebase copilot.** AST-aware chunking, safe repository tools
  (`search`/`read_file`/`propose_patch`), and an offline hybrid BM25+dense-style retriever
  are now implemented with qualified Python symbols and source spans; remaining work is
  cross-encoder rerank, MCP 2.1
  transport, planning
